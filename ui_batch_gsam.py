...
parser = argparse.ArgumentParser(
    prog="GSAMBatch", description="auto segmentation based on prompt"
)
parser.add_argument(
    "-gm",
    "--groundingdino_model",
...
parser.add_argument(
    "-gc",
    "--groundingdino_model_config",
    type=str,
    default="configs/groundingdino.py",
    help="config path",
)
parser.add_argument(
    "-sm",
...
parser.add_argument(
    "-st", "--sam_model_type", type=str, default="vit_h", help="sam model type"
)
parser.add_argument(
    "-o", "--output_dir", type=str, default="sample_outputs", help="output directory"
)
parser.add_argument(
    "-p",
    "--prompt",
    type=str,
    default="ships",
    help="search string delimited with commas",
)
...
grounding_dino_model = Model(
    model_config_path=args.groundingdino_model_config,
...
class GSAMBatch:
    def __init__(self, root):
        ...
        self.label_button = tk.Button(
            root, text="Label Images", command=self.label_image_gsam
        )
    ...
    def label_image_gsam(self):
        ...
        for filepath in self.filepaths:
            ...
            with open(f"{outpath}.bbox-points.txt", "w") as f:
            ...
            for idx, mask in enumerate(detections.mask):
                cv2.imwrite(outpath + f"{idx+1}-mask.png", mask * 255)
